class Members::ExamsController < ApplicationController
  ...
  def index
    @exams = @lesson.exams.of_member(current_user.id).recent.page(params[:page])
      .per Settings.per_page.members.exam
    if @exams.blank?
  ...
  def load_lesson
    ...
    redirect_to members_course_lessons_path(@course)
  end
  ...
  def load_course
    ...
    redirect_to members_courses_path
  end
